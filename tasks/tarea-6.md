...
R: Imagino que es una base de datos que guarda en local los datos creados por diferentes archivos
2. ¿Que diferencia hay entre `db.session.add()` y `db.session.commit()`?
R: Una sube un cambio y otra guarda en local el cambio, el .add seria el guardado local
3. ¿Por que `unique=True` es importante en el email?
R: Para que no se repita el email
